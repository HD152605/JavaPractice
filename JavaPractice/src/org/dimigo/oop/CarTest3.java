/**
 * 
 */
package org.dimigo.oop;

/**
 * <pre>
 * org.dimgo.oop
 *    |_ CarTest
 *
 * 1. 개요 :
 * 2. 작성일 : 2017. 3. 23.
 * </pre>
 * 
 *
 * @author          :김민선
 * @version         : 1.0
 */
public class CarTest3 {

	/**
	 * @param args
	 */

public static void main(String[] args) {
	
	//new 연산자를 이용햐서 객체를 생성함
	//객체의 주소를 저장하기 위해 Reference변수(book)를 선언함	
	Car3 car1 = new Car3("현대자동차","제네시스","검정색",225,50000000);
	
	//Ref 변수를 이용하여 객체에 접근하기
    System.out.println("<<자동차 목록>>");
	System.out.println("제조사명 : " + car1.getCompany());
	System.out.println("모델명 : " + car1.getModel());
	System.out.println("색상 : " + car1.getColor());
	System.out.println("최대속도 : " + car1.getMaxSpeed() + "km");
	System.out.println(String.format("가격 : " + "%,d", car1.getPrice())+"원");
	System.out.println();
	
	Car3 car2 = new Car3("기아자동차","K7","흰색",246);
/*	car2.setCompany("기아자동차");
	car2.setModel("K7");
	car2.setColor("흰색");
    car2.setMaxSpeed(246);
	car2.setPrice(40000000);
	*/
	
	System.out.println("제조사명 : " + car2.getCompany());
	System.out.println("모델명 : " + car2.getModel());
	System.out.println("색상 : " + car2.getColor());
	System.out.println("최대속도 : " + car2.getMaxSpeed() + "km");
	System.out.println(String.format("가격 : " + "%,d", car2.getPrice())+"원");
	System.out.println();
	
	Car3 car3 = new Car3("삼성자동차","SM7","회색");
	
	/*car3.setCompany("삼성자동차");
	car3.setModel("SM7");
	car3.setColor("회색");
    car3.setMaxSpeed(200);
	car3.setPrice(38000000);*/

	System.out.println("제조사명 : " + car3.getCompany());
	System.out.println("모델명 : " + car3.getModel());
	System.out.println("색상 : " + car3.getColor());
	System.out.println("최대속도 : " + car3.getMaxSpeed() + "km");
	System.out.println(String.format("가격 : " + "%,d", car3.getPrice())+"원");
	System.out.println();
}
}
